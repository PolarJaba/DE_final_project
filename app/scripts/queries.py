...
with connection.cursor() as cur:
    ...
    query = ("WITH costs_0p_t AS ( "
             "SELECT date_trunc('day', cd.trip_dropoff_datetime) AS \"date\", MAX(total_amount) AS max_cost_trip_0p, "
             "MIN(total_amount) AS min_cost_trip_0p "
             "FROM final.core_data cd "
             "WHERE passengers_count = 0 "
             "GROUP BY date_trunc('day', cd.trip_dropoff_datetime)), "
             "costs_1p_t AS ( "
             "SELECT date_trunc('day', cd.trip_dropoff_datetime) AS \"date\", MAX(total_amount) AS max_cost_trip_1p, "
             "MIN(total_amount) AS min_cost_trip_1p "
             "FROM final.core_data cd "
             "WHERE passengers_count = 1 "
             "GROUP BY date_trunc('day', cd.trip_dropoff_datetime)), "
             "costs_2p_t AS ( "
             "SELECT date_trunc('day', cd.trip_dropoff_datetime) AS \"date\", MAX(total_amount) AS max_cost_trip_2p, "
             "MIN(total_amount) AS min_cost_trip_2p "
             "FROM final.core_data cd "
             "WHERE passengers_count = 2 "
             "GROUP BY date_trunc('day', cd.trip_dropoff_datetime)), "
             "costs_3p_t AS ( "
             "SELECT date_trunc('day', cd.trip_dropoff_datetime) AS \"date\", MAX(total_amount) AS max_cost_trip_3p, "
             "MIN(total_amount) AS min_cost_trip_3p "
             "FROM final.core_data cd "
             "WHERE passengers_count = 3 "
             "GROUP BY date_trunc('day', cd.trip_dropoff_datetime)), "
             "costs_4p_t AS ( "
             "SELECT date_trunc('day', cd.trip_dropoff_datetime) AS \"date\", MAX(total_amount) AS "
             ...
             "max_cost_trip_2p, min_cost_trip_2p, max_cost_trip_3p, min_cost_trip_3p, "
             "max_cost_trip_4p_plus, min_cost_trip_4p_plus "
             "FROM (SELECT date_trunc('day', cd.trip_dropoff_datetime) AS \"date\" "
             "FROM final.core_data cd "
             "GROUP BY date_trunc('day', cd.trip_dropoff_datetime)) AS all_d "
